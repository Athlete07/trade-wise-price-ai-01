    ...
    <div className="space-y-6">
      ...
      {details.origin && details.destination && details.weight > 0 && details.value > 0 && (
        <Card className="bg-green-50 border-green-200">
          <CardHeader>
            <CardTitle className="text-green-900">Shipment Summary</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div>
                <p className="font-semibold text-green-900">Route:</p>
                <p className="text-green-800">{details.origin} → {details.destination}</p>
              </div>
              <div>
                <p className="font-semibold text-green-900">Weight:</p>
                <p className="text-green-800">{details.weight.toLocaleString()} kg</p>
              </div>
              <div>
                <p className="font-semibold text-green-900">Value:</p>
                <p className="text-green-800">{details.currency} {details.value.toLocaleString()}</p>
              </div>
              <div>
                <p className="font-semibold text-green-900">Value/kg:</p>
                <p className="text-green-800">{details.currency} {(details.value / details.weight).toFixed(2)}</p>
              </div>
            </div>
          </CardContent>
        </Card>
